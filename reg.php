    ...
    try {
        ...
        echo "<script>
            ...
            window.location.href='login.php';
                ...
                {
        // ADDED: show a clear alert if the UNIQUE constraint on EmailId is hit
        $isDuplicate = ($e->getCode() === '23000') || (!empty($e->errorInfo[1]) && $e->errorInfo[1] == 1062);
        if ($isDuplicate) {
            echo "<script>alert('This email is already registered. Please use another email.'); history.back();</script>";
        } else {
            echo "<script>alert('Error: Could not register. Please try again.'); history.back();</script>";
        }
        exit;
    }
